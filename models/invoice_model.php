...
function get_invoices($conn, $search='') {
    $search="%".$search."%";
    $stmt=$conn->prepare('SELECT i.*,p.patient_name FROM invoices i JOIN patients p ON p.id=i.patient_id WHERE i.invoice_no LIKE ? OR p.patient_name LIKE ? ORDER BY i.id DESC');
...
function find_invoice($conn,$id) { $stmt=$conn->prepare('SELECT i.*,p.patient_name,p.phone FROM invoices i JOIN patients p ON p.id=i.patient_id WHERE i.id=?'); $stmt->bind_param('i',$id); $stmt->execute(); return $stmt->get_result()->fetch_assoc(); }
?>
